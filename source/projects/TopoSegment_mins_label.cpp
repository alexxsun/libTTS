...
int TopoSegment::initSeeds_mins_by_as() {
    ...
    if (_showlog) cout << "init _min2lbl\n";
    ...
    if (_showlog) cout << "find connected mins in the low height\n";
    ...
    for (const auto &m: all_mins) {
        ...
        if (_showlog) cout << "check min: " << vid << endl;
        int tmp_lbl = lowpt2lbl[vid];
        _min2lbl[m] = tmp_lbl;
        if (_showlog) cout << "-> set as seed with id: " << tmp_lbl << endl;
    } // build seed_mins
    if (_showlog) {
...
int TopoSegment::initSeeds_mins_by_trunkpts_xyz(
    ...
    if (_showlog) std::cout << "seeds #: " << trunk_pts.size() << std::endl;
    ...
    for (const auto &m: all_mins) {
        ...
        if (bst_trunk_lbl != 0) {
            ...
            if (_showlog) cout << "-> set as seed with id: " << bst_trunk_lbl << endl;
        }
    } // build seed_mins
...
int TopoSegment::growFromSeeds_mins_basic() {
    ...
    if ((all_mins.size() - sorted_seed_mins.size()) == 0) {
        if (_showlog) cout << "all mins are seeds.\n";
    ...
    while (!sorted_seed_mins.empty()) {
        ...
        if (_showlog) cout << "remove processed seeds:\n";
        for (const auto &rm_id: rm_seed_ids) {
            if (_showlog) cout << rm_id << " ";
            sorted_seed_mins.erase(sorted_seed_mins.begin() + rm_id);
        }
        if (_showlog) cout << endl;

        // add seeds
        if (_showlog) cout << "add new seeds:\n";
        for (const auto &nbr: add_seeds) {
            if (_showlog) cout << nbr << " ";
