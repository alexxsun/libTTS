...
bool GradientEncoding::isPaired(const implicitS& simpl, explicitS& top, SimplicialComplex& sc) {
  ...
  switch (topDim) {
    case 1:
      ...
      if (vsimpl[0] == vtop[0]) return edgeLocalFrames[topIndex * 2];
...
void GradientEncoding::pair(const implicitS& big, const implicitS& small, explicitS& top, SimplicialComplex& sc) {
  ...
  if (top.getDim() == 1) { // 2D, edge.
  ...
  else if (top.getDim() == 2) { // 3D, triangle
    ...
    switch (smallDim) { // dimension of small simplex
      case 0:  // small: vertex
  ...
  switch (top.getDim()) {
    ...
    case 2:
      switch (simpl.getDim()) {
        case 0:
          pair =
              expandTriangleGrad(triangleLocalFrames[topIndex]).get_vertex_pair(getBitSet(simpl, sc.toImplicit(top)));
          break;
        case 1:
