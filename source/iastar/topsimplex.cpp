...
bool implicitS::operator!=(const implicitS& simpl) const { return !(*this == simpl); }

bool implicitS::contains(const implicitS& simpl) {
  vector<int> sVerts = simpl.getConstVertices();
  if (vertices.size() <= sVerts.size()) return false;
...
TopSimplex::TopSimplex(int vertex) { vertices.push_back(vertex); }

int TopSimplex::getDimension() { return vertices.size() - 1; }

int TopSimplex::get_nVertices() { return vertices.size(); }

vector<int>& TopSimplex::getVertices() { return vertices; }

int TopSimplex::getAdjacent(int faceIndex) const { return adjacents[faceIndex]; }

int TopSimplex::getVertexIndex(int vertex) const { return vertices[vertex]; }

void TopSimplex::setAdjacent(int faceIndex, int simpl) { adjacents[faceIndex] = simpl; }

void TopSimplex::print_debug() {
  for (uint i = 0; i < vertices.size(); i++) cout << vertices[i] << " ";
  cout << endl;
