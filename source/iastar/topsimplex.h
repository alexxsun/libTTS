...
// explicit representation for a simplex, only simplexes that are explicitly encoded (vertices and top-simplexes) can be
...
// implicit representation for a simplex based on a top-simplex. Every simplex in the simplicial complex can be
...
class implicitS {
 protected:              // protected Accessible to derived classes, ciccio used private here
 ...
 public:
  ...
  friend ostream& operator<<(ostream& os, const implicitS& simplex) {
    ...
    os << "[" << verts[0];
    for (int i = 1; i < verts.size(); i++) os << "," << verts[i];
    os << "]";
    return os;
  }
  // stream operator for printing out a simplex
...
class TopSimplex {
 private:
  ...
  vector<int> adjacents;  // set of adjacent top simplexes for each face, adjacents[i] contains an index to the
                          // top-simplexes adjacent on the ith face.
